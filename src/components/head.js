...
import { loginadd } from '../slice/loginslice';
...
function Head () {
  ...
  const popup = (placement ) => {
    api.info({
      // message: `Notification ${placement}`,
      description: <Context.Consumer>{({ name }) => `PLEASE SIGNUP`}</Context.Consumer>,
      placement,
    });
  };
  ...
  // }
  const login=values=>{
    axios.post("http://localhost:3000/api/v1/singup/find",{...values}
